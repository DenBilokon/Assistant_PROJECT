            ...
            for file in files:
                try:
                    ...
                    if list_files[-1].upper() in list_image:
                        new_name = os.path.join(root, SortDirectory.normalize_names(file))
                        ...
                        filename = SortDirectory.normalize_names(file)
...
def run_sort():
    try:
        ...
        SortDirectory.sort_create_files(start_path)
        while True:
            b = input("\nDo you want to sort something else? (yes/no)\n>>>> ")
            if b == "yes":
                a = str(input('Input path: '))
                SortDirectory.sort_create_files(a)
                continue
            if b == "no":
                print("Goodbye!")
                break
